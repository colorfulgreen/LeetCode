...
int leastInterval(char* tasks, int tasksSize, int n){
    ...
    int tasksCount[N_TASK_TYPES], i, interval = 0, total_interval = 0, last_interval = 0;
    for(i=0; i<N_TASK_TYPES; i++)
        tasksCount[i] = 0;
    for(i=0; i<tasksSize; i++) 
        tasksCount[tasks[i] - 'A']++;
    qsort(tasksCount, N_TASK_TYPES, sizeof(int), cmp);

    /* 完成最长任务所需的最少时间 */
    int longest = tasksCount[0];
    int min_runtime = (longest - 1) * (n + 1);
    for(int i=0; i<N_TASK_TYPES; i++) {
        if(tasksCount[i] == longest)
            min_runtime++;
        else
            break;
    }

    /* NOTE: 如果总时间片大于 min_runtime，则不需要插入 noop,
     * 因为 tasksCount 已经逆序排列了，后面任务需要插入的 noop 不会多于第一个任务。
     */
    return tasksSize > min_runtime ? tasksSize : min_runtime;
...
int main(void) {
    ...
    int n = 1;
    int interval = leastInterval(tasks, tasksSize, n);
    printf("%d\n", interval);
    return 0;
}
